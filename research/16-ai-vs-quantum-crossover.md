...
*Research date: 2026-09-07. Companion to `03-quantum-reality-check.md` (hardware status); this report tests a different thesis and avoids re-treading hardware roadmaps.*
...
6. **The reverse flow is real and underrated: AI is quantum's most valuable near-term supplier.** DeepMind's AlphaQubit 2 decodes distance-11 surface code in real time (<1 µs/cycle) and gives the first real-time color-code decoding at distance 9 [2]. Learned decoders, RL calibration and ML pulse control are on the critical path to fault tolerance. **[CONFIRMED]**
7. **Quantum's genuine, non-dequantizable moat is cryptanalysis** (Shor) — and there the news is that even the field's leading skeptic, Scott Aaronson, co-signed a 2026 position paper putting crypto-relevant machines around ~2029 [3]. **[REPORTED]**
...
**Learned decoders.** AlphaQubit (Nature, 2024) was a recurrent-transformer surface-code decoder more accurate than matching-based decoders but too slow for real time. **AlphaQubit 2** (arXiv 2512.07737, Dec 2025) closes that gap: near-optimal logical error rates for surface *and* color codes under realistic noise, real-time decoding **<1 µs per cycle** on commercial accelerators for the distance-11 surface code, better accuracy than leading real-time decoders, and the first real-time color-code decoding at distance 9 — orders of magnitude faster than other high-accuracy color-code decoders [2]. **[CONFIRMED — preprint from Google DeepMind + Google Quantum AI]**
...
There is no dequantization of Shor, no neural network that factors RSA-2048, and no classical algorithm in sight. This is quantum's one unambiguous, commercially consequential, non-negotiable advantage. The 2026 development that matters: Scott Aaronson — the field's most-cited skeptic, fresh off a two-year leave at OpenAI working on AI safety theory — published a May 2026 post and a co-authored position paper with Dan Boneh and Justin Drake putting crypto-relevant quantum computers around **~2029** [3]. **[REPORTED — a position paper, not a demonstration]** Gidney's factoring resource estimates have fallen by roughly an order of magnitude since 2019, which is the mechanism behind the date compression.
...
**Interpretation.** The ratio of annual capital is roughly **100–150:1** in AI's favor, and the ratio of *power* is larger still. Capital does not guarantee scientific success, but it does buy iteration speed, and in a race where both sides attack the same target (electronic structure), the side with 150× the resources and a 5-year head start on deployment wins the application layer by default. **[SPECULATION — this is my inference, high confidence]**
